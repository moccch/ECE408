...
__host__ void GPUInterface::conv_forward_gpu_prolog(float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    int segmentSize = B / numStreams;  
    int segmentSize_x = segmentSize * C * H * W;
    int segmentSize_y = segmentSize * M * H_out * W_out;
    for (int i = 0; i < numStreams; ++i) {
        int offset = i * segmentSize;
        cudaMemcpyAsync(*device_input_ptr + offset * C * H * W, 
                        host_input + offset * C * H * W, 
                        segmentSize_x * sizeof(float), 
                        cudaMemcpyHostToDevice, 
                        streams[i]);

        dim3 blockDim(TILE_WIDTH, TILE_WIDTH, 1);
        dim3 gridDim(segmentSize, M, ceil(static_cast<float>(W_out) / TILE_WIDTH) * ceil(static_cast<float>(H_out) / TILE_WIDTH));

        conv_forward_kernel<<<gridDim, blockDim, 0, streams[i]>>>(*device_output_ptr + offset * M * H_out * W_out, 
                                                                   *device_input_ptr + offset * C * H * W, 
                                                                   *device_mask_ptr, 
                                                                   B, M, C, H, W, K, S);
        cudaMemcpyAsync(host_output + offset * M * H_out * W_out, 
                        *device_output_ptr + offset * M * H_out * W_out, 
                        segmentSize_y * sizeof(float), 
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask_ptr, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    // dim3 gridDim(B, M, Y);
    // conv_forward_kernel<<<gridDim, blockDim>>>(device_output, device_input, device_mask_ptr, B, M, C, H, W, K, S);
